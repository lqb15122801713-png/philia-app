...
export default function TodayPage() {
  ...
  // 加载态：骨架（与两台骨架卡同风格，避免分流前闪屏）
  if (loading) {
    return (
      <div className="px-4 pb-6">
        <div className="pt-6">
          <div className="h-8 w-32 animate-pulse rounded-tag bg-sunken" />
          <div className="mt-2 h-5 w-48 animate-pulse rounded-tag bg-sunken" />
        </div>
  ...
  if (!user?.staffId) {
    return (
      <div className="flex min-h-[70vh] flex-col items-center justify-center px-6 text-center">
        <span
          aria-hidden
          className="flex h-24 w-24 items-center justify-center rounded-full bg-brand-primary-light text-5xl"
        >
          🐾
        </span>
