/**
 * 员工端 SSE 接线 hook（契约 docs/STAFF-CONTRACTS.md §通用约定 · T3.1）
 *
 * - clientId 复用 localStorage `philia.sseClientId`（与客户端同一键，跨端互不干扰——
 *   服务端按 user_id + client_id 登记订阅）；
 * - 先 push.subscribe（appType='staff'）登记，失败 5s 重试直到成功；
 * - 成功后连 GET /api/events?client_id=…（不带 watch：员工基础频道 user:{id} +
 *   staff:{staffId} 已覆盖 派单/改期/取消/重拍 事件，见 server/src/routes/events.ts）；
...
export function useStaffEvents(opts: {
