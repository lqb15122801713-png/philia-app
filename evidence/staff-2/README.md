...
- **R8**：stock_movements 地基（收银扣减/冲正回补/盘点/消毒四来源+前后值+操作人）；盘点状态机 draft→counted→confirmed→posted（驳回退回重盘）；单价≥100 日盘/周一全量周盘/盲盘标记；安心包单独成类+效期≤30 天预警只读；消毒步联动扣减落流水。
...
# 补充令①施工（2026-09-21 · 决策 #39/#40 · 时长系数表配置化）

- **断档核实**：产品侧补发原文（本卷宗收件为准），照单施工，同一分支 feat/staff-2。
- **duration_rules（迁移 0013）**：引擎六块系数（物种×种类基础时长/体型/毛长/体重分档/长毛品种词表/服务种类词表）全量从代码常量迁配置表，初始值照转占位、label 带"占位待供给"；引擎逻辑零变化只改取数来源（loadDurationRules 读 active=1，缺行回退服务默认 durationMin，参数零代码常量）。
- **配置端口第三页签"时长系数"**：嵌套对象/倍率/词表渲染族+占位告示条；保存即生效+版本化前后值留痕；新值只管新预约（scheduledEnd 创建时落库不回溯，e2e 实证）。
- **G0 挂钩（决策 #40）**：isWashService 改读 duration_service_kind_keywords（提成与时长共表一处维护）；G0 规则行 valueJson {rate_bp, scope}，scope 默认 bath=仅洗护、老板端口枚举（bath/all）可调、缺省向后兼容。
- **边界**：三端时长口径对齐未做（归价目表批次）；cashier.ts 未引引擎未动。
- **闸门复跑**：build exit 0 / typecheck exit 0 / e2e exit 0（新增 §25 双向断言 10 项：改体型系数→新预约按新值 90→120min 精确/旧单 90min 不变/留痕前后值；G0 scope=bath 造型不计→scope=all 计提 5%）/ smoke-routes 47/47 / smoke-deploy 55 项（日志已刷新为本轮）。既有时长 smoke 不破（domain.smoke 清场补 xp_events=既有 bug 修复在案）。